...
Function Build-Setups
{
    Write-Host "Building setups"

    $Target = $Global:Distrib + "\Setups"
    $NuGetSite = (Get-Config-Parameter "nuGetSite")
    ...
    foreach ($Item in (Get-ChildItem -Path ($Global:Root + "\packages") -Filter "NetIde.Support.*"))
    {
        $MakeSetup = $Item.FullName + "\Tools\Setup\nimakesetup.exe"
    }

    & $MakeSetup /t "Net IDE Demo" /c NetIdeDemo /p NetIdeDemo.Package.Core /o ($Target + "\Net IDE Demo.exe") /n $NuGetSite
    & $MakeSetup /t "Net IDE Demo Plugin" /c NetIdeDemo /p NetIdeDemo.Package.Plugin /o ($Target + "\Net IDE Demo Plugin.exe") /n $NuGetSite
...
{
    Build-Solution -Solution ($Global:Root + "\NetIde Demo.sln") -Configuration $Global:Configuration
